...
The Mean is \$355,988, when the Median is $280,000. That signals that the data is skewed. Let's review the boxplot and histograms of sales data. Since data is right-skewed, we test the effect of a logarithmic transformation on the *Sales Price*.
...
Application of Machine Learning algorithms for predicting property values is reasonable in a mass appraisal setting. It offers flexibility and scalability necessary when valuing a large number of properties on an annual basis. The limitations include initial investment in data pre-processing and feature engineering, and continuous need for tuning of the model. While Ensemble models offer better performance when compared to most individual models, they could be complex in its application and interpretability. 

The best model produced through this work is a weighted ensemble model based on Random Forest, Gradient Boosting, and Deep Learning (DL). This model yields the lowest MSE of 0.0189163 with results deviating by \$54,235 on average and $33,230 using mean. There is a wide array of opportunities to continue tuning parameters and changing the structure of the Deep Learning model. The biggest challenge using the DL model is its lack of interpretability for assessors working in the highly regulated industry. 

The future work includes: (1) Continued feature engineering, adding new variables or removing features that don't improve the model; (2) Parameter and DL structure tuning in addition to the utilization of GPU training clusters; (3) Evaluation of additional models and ensembling options. 
