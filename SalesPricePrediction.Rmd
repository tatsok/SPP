...
Let's visualize our tidy data set on Orange County, Florida map using *Leaflet* package.
...
# labels <- sprintf("Date of Sale:<strong> %s</strong><br/>Sale Price: $%g<br/>
                  #Heated Area: %g<br>Beds: %g <br>Baths: %g", sqtest$Sale_Date, 
                  #sqtest$Sales_Price, sqtest$HEATED_AREA, sqtest$BEDS, 
                  #sqtest$BATHS) %>% lapply(htmltools::HTML)
...
Let's review the boxplot and histograms of sales data. Since data is right-skewed, we test the effect of a logarithmic transformation on the *Sales Price*.
...
sqtest<-sqtest%>%mutate(AREA=ifelse(AREA==0, 1, AREA))%>%
  mutate(STORIES=ifelse(is.na(STORIES),1,STORIES), BEDS=ifelse(is.na(BEDS),1,BEDS), BATHS=ifelse(is.na(BATHS),1,BATHS))%>%mutate(Sales_Price=log(Sales_Price),
                                                                                                                                 AREA=log(AREA), HEATED_AREA=log(HEATED_AREA), Land_Sq=log(Land_Sq))
```
...
Generalized Linear Model (GLM), perhabs most commonly used model, represents our outcome variable as a linear function of input features. To improve model's performance, we will define trControl function that controls the resampling scheme to 10-fold cross-validation:
...
Loess model is a local polynomial regression that uses local weighted regression to fit a smooth curve through data points. We use 5-fold cross-validation for this model. In Loess, we can tune the parameters, namely span, using tuneGrid function. Tuning is omitted from this work to improve the computational time.
...
Yet, the MSE results have improved from 0.0495869 to 0.0347857. 
...
The SVM model showed an increase in MSE when compared to GLM or Loess. The varImp function showed that *HEATED_AREA*, *BATHS*, *POOL* are among the top three variables, while *SHED*, *DOR_CODE*, and *SHED_N* is the least important.
...
Similarly to the svmLinear model, *HEATED_AREA* is by far more important than the rest of the variables. By plotting the model, we see that the best number of neighbors parameter is 5.
...
Since predFixed=10 and  minNode=1 are the best parameters, we apply these using the Rborist model with 500 trees, which a faster implementation of Random Forest:
...
By plotting the most influential parameters, we see that *HEATED_AREA* appears to have the most relative influence followed by *DOR_CODE* and *AREA*..
...
The best result was achieved with *i* equal to 1. Let's recreate the ensemble 2 model:
...
Using the powerful *Keras* package, we create a network with five dense layers with the following units: 128, 512, 256, 128, 1 (output layer). And a dropout rate of 0.1 that helps to reduce overfitting. The network ends with a single unit to predict a single continuous value of Sales Prices.
...
build_model<-function(){
  model <- keras_model_sequential() %>%
    ...
    layer_dropout(rate=0.1)%>%layer_dense(units = 128, activation = "relu")%>%
...
According to the plot, MSE stops improving after 70 epochs. Past that, we start overfilling. While not included in this work, the size and number of hidden layers can also be tuned to improve the model's performance.

Now let's train the final deep learning model with the best parameters (in this case, the number of epochs is 70). *Please note that when testing on GPU, the model yields a better result. The result discussed in this study was produced on CPU as proof of concept only. GPU use is highly recommended.* The model fit using Tesla V100 GPU on a ubuntu-18.04-LTS server.
...
Obtaining a reproducible model that yields the same MSE is challenging with *Keras.* In order to maintain a consistent result, the original model produced ("my_dl_model") is also available in the repository.

```{r dlev, include=TRUE, message=TRUE, warning=FALSE}
model%>%evaluate(test_x, test_y)
model <- load_model_hdf5('my_dl_model') #the original model
...
First, we assign the same weight for each model.
...
That means that the model's results deviate by 55 thousand dollars from the average and 33 thousand from the mean Sales Price.

# INDUSTRY STANDARDS

Now, let's evaluate if this model passes the assessment industry standards. First, we will apply exponential transformation to the predicted and actual results; then, apply 85% legal requirement to the Predicted (Assessment) values before calculating the hypothetical Sales Ratio:
...
We can see from the plot, the distribution is fairly close to the original Sales Prices considering the 15% adjustment.
...
Let's compare the performance of the final model ratios to the original data ratios. 

```{r ofstat, include=TRUE, message=TRUE, warning=FALSE}
original
```

All ratios have either improved or within acceptable limits. The MEDIAN and MEAN of the final model are closer to 85%. The COD and PRD of the data are within acceptable industry ranges. 95% of the observations are within the 61 to 110 range, which is a bit above the original due to sampling. We can conclude that the model passes the industry standards test.

\newpage
## CONCLUSION AND FUTURE WORK

Application of Machine Learning algorithms for predicting property values is reasonable in a mass appraisal setting. It offers flexibility and scalability necessary when valuing a large number of properties on an annual basis. The limitations in this technique include time spent on feature engineering and tuning of the parameters for each model. Ensemble models offer better performance when compared to most individual models, but could be complex in its application and interpretability. 

The best model produced through this work is a weighted ensemble model based on Random Forest, Gradient Boosting, and Deep Learning (DL). This model yields the lowest MSE with results deviating by $55 thousand$ from the average and $33 thousand$ from the mean Sales Price. There is a wide array of opportunities to continue tuning parameters and changing the structure of the Deep Learning model. The biggest limitation of the DL model is its lack of interpretability for assessors working in the highly regulated industry. 

The future work includes: (1) Continued feature engineering including wrangling, removing or adding new variables with a focus on reducing correlation between features; (2) Parameter and DL structure tuning in addition the utilization of GPU training clusters; (3) Evaluation of additional models and ensembling options. 
